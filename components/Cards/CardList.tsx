...
const CardListComponent = () => {
	...
	return (<div className="flex justify-around">{cards.map(item => <CardComponent badge={item.badge} title={item.title} hashtag={item.hashtag} price={item.price} imagesrc={item.imagesrc} imagetitle={item.title} imagewidth={430} imageheight={524} />)}</div>);
}
